...
| `cacheKey` | `string` | Override the default cache key (defaults to `JSON.stringify([pathname, trimmed input, language, languageHints, type, length, format, preference, sharedContext])`; `languageHints` is a boolean derived from `supportedLanguages`). |
