...
- **Explicit stream compatibility.** Native chunks default to deltas. For verified snapshot hosts, set `streamMode: "cumulative"`. `onUpdate` stays cumulative, and `sendStreaming()` yields deltas. Automatic prefix detection is no longer used.
