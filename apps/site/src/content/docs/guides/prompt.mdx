...
- **Native stream chunks.** The wrapper concatenates each native chunk. Repeated chunks remain separate, `onUpdate` receives the accumulated text, and `sendStreaming()` yields deltas.
