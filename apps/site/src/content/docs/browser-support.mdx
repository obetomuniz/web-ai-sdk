...
| `@web-ai-sdk/writer` | 137+ · OT · flag | 138+ · Canary/Dev | no-op fallback | no-op fallback |
| `@web-ai-sdk/rewriter` | 137+ · OT · flag | 138+ · Canary/Dev | no-op fallback | no-op fallback |
| `@web-ai-sdk/proofreader` | 141+ · OT · flag | 142+ · Canary/Dev | no-op fallback | no-op fallback |
...
Only the Summarizer ships in Edge stable (enabled by default since Edge 138, per the [Edge Writing Assistance APIs docs](https://learn.microsoft.com/en-us/microsoft-edge/web-platform/writing-assistance-apis)). The Prompt API is a developer preview in Edge Canary/Dev 138+, and Translator (Edge 143+) and Language Detector (Edge 147+) are Canary/Dev-only as well, each behind its own `edge://flags/` toggle. The Writer and Rewriter APIs are developer previews in Edge Canary/Dev 138+, and the Proofreader API in Edge Canary/Dev 142+, each behind its own "… API for Phi mini" flag. WebMCP landed in Edge 147 behind a flag.
...
- Flags on `edge://flags/`: "Prompt API for Phi mini", "Writer API for Phi mini", "Rewriter API for Phi mini", "Proofreader API for Phi mini", "translation api" (Translator), "language detection" (Language Detector). The Summarizer is on by default since Edge 138.
