...
## Disable WebMCP where tools do not belong

Send this response header with each document that must not expose WebMCP tools:

```http
Permissions-Policy: tools=()
```

The empty allowlist disables WebMCP for the document and all descendant frames. The header is defense in depth. It does not authorize tool calls, request confirmation, or prevent prompt injection. See the [WebMCP guide](/docs/guides/webmcp/#permissions-policy).
