...
`@web-ai-sdk/all` and the eight other scoped packages release together. Every release ships all nine at the same version via a Changesets `fixed` group, so pinning a single version in your `package.json` gets you the whole suite at a known state.
