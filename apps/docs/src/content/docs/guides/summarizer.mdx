---
title: Summarizer
description: Building block for the Web's Built-in Summarizer with session reuse, opt-in result caching, and streaming.
---
...
| `cacheKey` | `string` | Override the default cache key (defaults to `${pathname}:${lang}`). |
