...
When the underlying Summarizer instance exposes `summarizeStreaming()`, the library uses it and pushes cleaned chunks to `onChunk` as they arrive. Otherwise it falls back to one-shot `summarize()`. Either way, `result.summary` is the final cleaned text.
